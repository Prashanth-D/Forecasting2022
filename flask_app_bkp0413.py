...
def getDrugByDisease():
    ...
    if ((requestJSON is not None) and ("selected_disease" in requestJSON) and ("selected_country" in requestJSON) and (
        ...
        # print(selected_country1)  #print(selected_disease)
        drugsToSelect = []
        conn = connect_db()
        ...
        if selected_country == "Lao People's Democratic Republic":
            ...
            drug_obj_by_cntry = conn.execute(
                "select distinct drug from " + countryDrug_year + " where country='" + selected_country + "' and score>0").fetchall()
...
def getCompanyByDrug():
    ...
    if ((rjson is not None)):
        # and rjson.keys() >= KeysFirstThird  (condition from above if)
        selected_country = request.get_json()["selected_country"]
        selected_disease = request.get_json()["selected_disease"]
        drug = request.get_json()["selected_drug"]
...
def getResults():
    ...
    if (selected_disease == "HIV"):
        ...
        newI_a1 = (adult_daly * adult_coverage * (first_adult_distribution * first_adult_prop * first_adult_eff)) / (
                    1 - adult_coverage * (first_adult_distribution * first_adult_prop * first_adult_eff))
        newI_a2 = (adult_daly * adult_coverage * (second_adult_distribution * second_adult_prop * second_adult_eff)) / (
                    1 - adult_coverage * (second_adult_distribution * second_adult_prop * second_adult_eff))
        newI_c1 = (child_daly * child_coverage * (first_child_distribution * first_child_prop * first_child_eff)) / (
                    1 - child_coverage * (first_child_distribution * first_child_prop * first_child_eff))
        newI_c2 = (child_daly * child_coverage * (second_child_distribution * second_child_prop * second_child_eff)) / (
                    1 - child_coverage * (second_child_distribution * second_child_prop * second_child_eff))
        ...
        if (selected_state == 'Company'):
            company_obj = conn.execute("select company,hiv from drugr2015 where hiv > 0").fetchall()
            # print(countries_obj[0])
            company_dict = {}
            for company in company_obj:
                temp = company[0].lstrip();
    ...
    if (selected_state == 'Country'):
        # countries_obj = conn.execute("select country,whipworm from country2015").fetchall()
        countries_obj = conn.execute(
            "select country,score from countryDrug2015 where drug='" + selected_drug + "' and score > 0.0").fetchall()
        countries_dict = {}
        for countries in countries_obj:
            temp = countries[0].lstrip();
            if (temp != selected_country):
