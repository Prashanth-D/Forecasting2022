...
conn.execute('''DROP TABLE IF EXISTS spec_treatment_regimen_2013''')
conn.execute('''CREATE TABLE spec_treatment_regimen_2013
...
treat_reg_tb_hiv_plus = list(df_impact_score.iloc[1,1].split(","))
...
treat_reg_tb_hiv_minus = list(df_impact_score.iloc[1,2].split(","))
...
treat_reg_mdr = list(df_impact_score.iloc[1,3].replace('+',',').replace('(or)',',').split(','))
drug = drug + list(set(treat_reg_mdr) - set(drug))

treat_reg_mdr1 = list(df_impact_score.iloc[1,4].replace('+',',').replace('(or)',',').split(','))
treat_reg_mdr = treat_reg_mdr + list(set(treat_reg_mdr1) - set(treat_reg_mdr))

treat_reg_mdr2 = list(df_impact_score.iloc[1,5].replace('+',',').replace('(or)',',').split(','))
treat_reg_mdr = treat_reg_mdr + list(set(treat_reg_mdr2) - set(treat_reg_mdr))
drug = drug + list(set(treat_reg_mdr) - set(drug))

treat_reg_xdr = list(df_impact_score.iloc[1,6].replace('+',',').replace('(or)',',').split(','))
...
sortedlist = sorted(drug2013, reverse=False)
for row in sortedlist:
    # print(row)
    conn.execute('insert into spec_treatment_regimen_2013 values (?,?,?,?,?,?,?)', row)
conn.commit()
conn.close()
