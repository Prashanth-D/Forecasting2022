...
conn.execute('''DROP TABLE IF EXISTS spec_treatment_regimen''')
conn.execute('''CREATE TABLE spec_treatment_regimen
...
for row in sortedlist:
    # print(row)
    conn.execute('insert into spec_treatment_regimen values (?,?,?,?,?,?,?)', row)
conn.commit()
conn.close()
