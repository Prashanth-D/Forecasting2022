...
def createTablesSpec():
    ...
    for year in years:
        conn.execute(''' DROP TABLE IF EXISTS spec_{}_NTD '''.format(year))
        conn.execute(''' CREATE TABLE spec_{}_NTD
        ...
        LF_prevalance real, LF_coverage real, LF_Efficacy_DEC real, LF_Efficacy_DEC_ALB real, LF_Efficacy_IVM_ALB real,LF_DALY real) '''.format(year))
    conn.close()
    return
...
def uploadSpecData():
    ...
    try:
        ...
        for year in years:
            # print(year)
            url = "ORS_Forecasting_NTD_{}.csv".format(year)
            ...
            for i in range(1, 218):
                ...
                spec_NTD_data.append(row)
            # print(spec_NTD_data[0])
                
            for k in spec_NTD_data:
                # print(k)
                conn.execute(''' INSERT INTO spec_{}_NTD VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) '''.format(year), k)
