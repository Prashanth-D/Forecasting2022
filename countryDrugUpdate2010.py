...
def createTablesCountry():
    ...
    conn.execute(''' DROP TABLE IF EXISTS countryDrug2010 ''')
    conn.execute(''' CREATE TABLE countryDrug2010 (country text, drug text, company text, score real) ''')
...
def countrydbUpdate():
    ...
    try:
        ...
        #         # print(drugToMatch)
        #         company = conn.execute("select distinct drug,company from drugr2010 where drug like'%" + drugToMatch[
        ...
        for country in countryDrugData:
            for drugs in (countryDrugData[country]):
                ...
                company = conn.execute(
                    "select distinct Drug,Company from results_impact_map2010 where Drug=trim('" + drugs + "') COLLATE NOCASE").fetchall()
        ...
        for k in countryDrugDataFinal:
            conn.execute(''' INSERT INTO countryDrug2010 VALUES (?,?,?,?) ''', k)
        conn.commit()
        conn.close()
