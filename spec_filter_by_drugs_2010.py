...
conn.execute('''DROP TABLE IF EXISTS spec_treatment_regimen_2010''')
conn.execute('''CREATE TABLE spec_treatment_regimen_2010
...
sortedlist = sorted(drug2010, reverse=False)
for row in sortedlist:
    # print(row)
    conn.execute('insert into spec_treatment_regimen_2010 values (?,?,?,?,?,?,?)', row)
conn.commit()
conn.close()
