...
def createTablesCountry():
    ...
    conn.execute(''' DROP TABLE IF EXISTS countryDrug2013 ''')
    conn.execute(''' CREATE TABLE countryDrug2013 (country text, drug text, company text, score real) ''')
...
def countrydbUpdate():
    ...
    try:
        ...
        #         # print(drugToMatch)
        #         company = conn.execute("select distinct drug,company from drugr2013 where drug like'%" + drugToMatch[
        #             0] + "%'COLLATE NOCASE").fetchall()
        #         cmpny = ""
        #         if (len(company) > 0):
        #             cmpny = company[0][1]
        #
        ...
        for country in countryDrugData:
            for drugs in (countryDrugData[country]):
                ...
                company = conn.execute(
                    "select distinct Drug,Company from results_impact_map2013 where Drug=trim('" + drugs + "') COLLATE NOCASE").fetchall()
                ...
                if(len(company) >0 ):
                    ...
                    country_to_upd = country.strip()
                    # row = [country,drugs,cmpny,countryDrugData[country][drugs]]
                    if (score_to_upd > 0):
                        row = [country_to_upd, drug_from_result, cmpny, score_to_upd]
                        countryDrugDataFinal.append(row)

        for k in countryDrugDataFinal:
            conn.execute(''' INSERT INTO countryDrug2013 VALUES (?,?,?,?) ''', k)
        conn.commit()
        conn.close()
