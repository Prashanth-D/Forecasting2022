...
mapping_2010 = {2: "TB", 3: "Malaria", 4: "HIV", 5: "Roundworm", 6: "Hookworm", 7: "Whipworm", 8: "Schistosomiasis",
           9: "Onchoceriasis", 10: "LF"}
...
def getCountries():
    ...
    # SEQUENCE 1
        ...
        conn = connect_db()
        countries_obj = conn.execute("select country from " + country_year + " where total > 0").fetchall()
        list_of_countries = [i[0] for i in countries_obj]
        return json.dumps(list_of_countries)
...
def getDrugByDisease():
    ...
    if ((requestJSON is not None) and ("selected_disease" in requestJSON) and ("selected_country" in requestJSON) and (
        ...
        elif "d'Ivoire" in selected_country:
            drug_obj_by_cntry = conn.execute(
                "select distinct drug from " + countryDrug_year + " where country like '%Ivoire%' and score>0").fetchall()
        elif selected_country == "Democratic People's Republic of Korea":
            drug_obj_by_cntry = conn.execute(
                "select distinct drug from " + countryDrug_year + " where country like 'Democratic People%' and score>0").fetchall()
        else:
            drug_obj_by_cntry = conn.execute(
            ...
            for drgIdxByDis, drgByDis in enumerate(drug_obj_by_dis):
                for drgIdxByCnt, drgByCnt in enumerate(drug_obj_by_cntry):
                    ...
                    if (drgByDis[0].lower() == drgByCnt[0].lower()):
                        drugsToSelect.append(drgByDis[0])
            # print(drugsToSelect)
            if(len(drugsToSelect) > 0):
                return json.dumps(drugsToSelect)
        print("No matching vaues")
        results_impact_map = "results_impact_map" + str(selected_year)
        print('''select distinct Drug from "%s" where Country = "%s" and Disease = "%s"''' %(results_impact_map, selected_country, selected_disease))
        drugs_impact = conn.execute('''select distinct Drug from "%s" where Country = "%s" and Disease = "%s"''' %(results_impact_map, selected_country, selected_disease)).fetchall()
        print(drugs_impact)
        for drugsIdx, drugsByValue in enumerate(drugs_impact):
            drugsToSelect.append(drugsByValue[0])
        print(drugsToSelect)
        return json.dumps(drugsToSelect)
        #return "No data found"
    # SEQUENCES 2 and 4
...
def getCountrybyDrug():
    ...
    results_impact_map_year = "results_impact_map" + str(request.get_json()["selected_year"])
    ...
    if(len(countries_obj) == 0):
        countries_obj = conn.execute("select Country from " + results_impact_map_year + " where LOWER(Drug) = '" + drug + "' AND Impact > 0").fetchall()
...
def getDiseaseFromCountryandDrug():
    ...
    diseases = []
    print(country_select)
    print('''select * from "%s" where trim(country) = trim("%s") ''' %(country_year, country_select))
    #countries_obj = conn.execute('''select * from "%s" where country = "%s" ''' %(country_year, country_select)).fetchall()
    #print(len(countries_obj))
    #if(len(countries_obj) == 0):
     #   countires_obj  = conn.execute('''select Disease from "%s" where Country = "%s" ''' %(results_impact_map_year, country_select)).fetchall()
    
    #print(countries_obj)
    if country_select == "Lao People's Democratic Republic":
        print("Here")
        countries_obj = conn.execute("select * from " + country_year + " where country like ' Lao%' ").fetchall()
    elif country_select == "Cote d'Ivoire":
        countries_obj = conn.execute(
            "select * from " + country_year + " where country like 'Cote d%' ").fetchall()
    else:
        print("select * from " + country_year + " where country = " + "' " + str(country_select) + "'")
        countries_obj = conn.execute(
            "select * from " + country_year + " where trim(country) = trim(" + "' " + str(country_select) + "')").fetchall()           
...
def getCompanyByDiseaseAndCountryAndDrug():
    ...
    company_obj = conn.execute(
        '''select TRIM(company) from "%s" where score > 0 AND trim(LOWER(drug)) = "%s" AND trim(LOWER(country)) = "%s" ''' %(countryDrug_year, drug.lower(), country.lower())).fetchall()
    #company_obj = conn.execute(
    ...
    if (len(companies_obj) > 1):
        compDisObj = conn.execute(
            "select TRIM(company) from " + drug_year + " where score > 0 AND LOWER(drug) ='" + drug.lower() + "' AND LOWER(disease) ='" + disease.lower() + "'").fetchall()
        print(compDisObj)
        ...
        compDisObj = conn.execute(
            "select distinct TRIM(Company) from " + results_impact_map_year + " where Impact > 0 AND LOWER(Drug) ='" + drug.lower() + "' AND LOWER(Disease) ='" + disease.lower() + "'").fetchall()
        print(compDisObj)
        compDisObj = [i[0] for i in compDisObj]
        companies_obj = list(set(set(compDisObj)))
        companies_obj_final = companies_obj
    
    

    print(companies_obj_final)
...
def getCountryByCompany():
    ...
    if company_select == "Kyorin Pharmaceutical Co., Ltd":
        company_select = "Kyorin Pharmaceutical Co., Ltd."
    if company_select == "Abbot Laboratories":
        company_select = "Abbott Laboratories"
    #if company_select == "Taisho Pharmaceuticals":
     #   company_select = "Taisho Pharmaceutical"
    ...
    print("select distinct country from " + countryDrug_year + " where trim(company)='" + company_select + "' COLLATE and score > 0")
    country_obj = conn.execute(
       "select distinct country from " + countryDrug_year + " where trim(company) = trim('" + company_select + "') and score > 0").fetchall()
...
def getDiseaseFromCountryCmpny():
    ...
    conn = connect_db()
    disease = []
    print('''select * from "%s" where country = "%s" ''' %(country_year, str(country_select)))
    #countries_obj = conn.execute(
       #'''select * from "%s" where country = "%s" ''' %(country_year, str(country_select))).fetchall()
    if country_select == "Lao People's Democratic Republic":
        print("Lao")
        countries_obj = conn.execute(
            '''select * from ''' + country_year + ''' where country like " Lao People's Democratic Republic" ''').fetchall()
    elif country_select == "Democratic People's Republic of Korea":
        print("Korea")
        countries_obj = conn.execute(
            ''' select * from ''' + country_year + ''' where country = "Democratic People's Republic of Korea" ''').fetchall()
    else:
        countries_obj = conn.execute(
            "select * from " + country_year + " where country = " + "' " + str(country_select) + "'").fetchall()
    company_obj = conn.execute(
        "select distinct disease from " + drug_year + " where trim(company) = trim('" + company_select + "')").fetchall()
    print(company_obj)
    print(countries_obj)
      ...
      #      if (val > 0):
       #         print(idx, val)
        #        for cmpnyIdx, cmpnyDisease in enumerate(company_obj):
         ...
         #   return json.dumps(disease)
    #else:
    disease_impact = conn.execute('''select distinct Disease from "%s" where Country = "%s" AND Company = "%s" '''%(results_impact_map_year, str(country_select), company_select)).fetchall()
    print(disease_impact)
    for disIdxByDis, disByDis in enumerate(disease_impact):
        disease.append(disByDis[0])
    print(disease)
    if(len(disease) > 0):
        return json.dumps(disease)
    return json.dumps("No diseases found")
...
def getDrugByDiseaseCmpny():
    ...
    drug_obj_by_cntry = conn.execute(
        '''select distinct drug from "%s" where ltrim(country) = "%s" and score > 0 ''' %(countryDrug_year, selected_country)).fetchall()
    #if selected_country == "Lao People's Democratic Republic":
     #   drug_obj_by_cntry = conn.execute(
     #       "select distinct drug from " + countryDrug_year + " where country like 'Lao People%' and score>0").fetchall()
    #else:
    ...
    if (len(drug_obj_by_cntry) > 0):
        ...
        if(len(drugsToSelect) == 0):
            ...
            if selected_disease == 'lf':
                selected_disease = 'LF'
            print(selected_disease)
            drugs_impact = conn.execute('''select distinct Drug from "%s" where trim(Country) = "%s" AND Disease = "%s" AND Company = "%s"'''%(results_impact_map_year, selected_country, selected_disease, selected_company)).fetchall()
            print(drugs_impact)
            for drgIdxByDis, drgByDis in enumerate(drugs_impact):
                    drugsToSelect.append(drgByDis[0])
            print(drugsToSelect)
            return json.dumps(drugsToSelect)
        else:
...
def getSpecification():
    ...
    # print(selected_drug)
    # selected_company = request.get_json()["selected_company"]
    spec_year_HIV_Regimen = "spec_" + str(selected_year) + "_HIV_Regimen"
    drug_year = "drug" + str(selected_year)
    ...
    conn = connect_db()
    spec_obj = ""
    spec_treatment_regimen_year = "spec_treatment_regimen_" + str(selected_year)
    if (selected_disease == 'TB' or selected_disease == 'Malaria'):
        # print("get TB regimen")
        spec_obj = conn.execute(
            "select tb_hiv_plus,tb_hiv_minus,tb_mdr,tb_xdr,malaria_PFalc, malaria_PVivax from " + spec_treatment_regimen_year + " where trim(drug) = '" + sd + "' COLLATE NOCASE").fetchall()
        print(spec_obj)
    elif (selected_disease == 'HIV'):
        spec_obj = conn.execute(
            "select Ndrug,Adult_prop,Children_prop,Adult_eff,Children_eff from " + spec_year_HIV_Regimen + " where Regimens like '%" + sd + "%' COLLATE NOCASE").fetchall()
    spec_obj_NTDs = conn.execute(
        "select disease from " + drug_year + " where drug ='" + selected_drug + "' COLLATE NOCASE").fetchall()
    # print(spec_obj_NTDs)
    ...
    if (len(spec_obj) > 0):
        ...
        # use this code if they want specifications to be as PFalc and PVivax for Malaria
            ...
            # HIV 2013 and 2010 use WHO groups for impact scores
                ...
                if 'Côte' in selected_country:
                    spec_obj = conn.execute(
                        "select * from spec_" + selected_year + "_HIV where country like 'Cote%' COLLATE NOCASE").fetchall()
                else:    
                    spec_obj = conn.execute(
                        "select * from spec_" + selected_year + "_HIV where country ='" + selected_country + "' COLLATE NOCASE").fetchall()
...
def getSpecTB():
    ...
    #print(f'''select * from {spec_year_TB} where country = '{selected_country}' COLLATE NOCASE''')
    if selected_country == "Republic of the Congo":
        selected_country = "Congo"
    spec_obj = conn.execute(
        '''select * from "%s" where country = "%s" COLLATE NOCASE''' %(spec_year_TB, selected_country)).fetchall()
    #spec_obj = conn.execute(
    #    f'''select * from {spec_year_TB} where country = '{selected_country}' COLLATE NOCASE''').fetchall()
    spec_treatment_regimen_year = "spec_treatment_regimen_" +str(selected_year)
    specdrug_obj = conn.execute(
        "select tb_hiv_plus,tb_hiv_minus,tb_mdr,tb_xdr from " + spec_treatment_regimen_year + " where drug ='" + sd + "' COLLATE NOCASE").fetchall()
...
def getSpecMalaria():
    ...
    print('''select * from "%s" where country = "%s" COLLATE NOCASE''' %(spec_year_Malaria, selected_country))

    spec_obj = conn.execute(
        '''select * from "%s" where country = "%s" COLLATE NOCASE''' %(spec_year_Malaria, selected_country)).fetchall()
    #if "Lao" in selected_country:
    #    spec_obj = conn.execute(
    #        "select * from " + spec_year_Malaria + " where country like 'Lao%' COLLATE NOCASE").fetchall()
    #else:
    #    spec_obj = conn.execute(
    #       "select * from " + spec_year_Malaria + " where country ='" + selected_country + "' COLLATE NOCASE").fetchall()
    spec_treatment_regimen_year = "spec_treatment_regimen_" + str(selected_year)
    specdrug_obj = conn.execute(
        "select malaria_PFalc, malaria_PVivax from " + spec_treatment_regimen_year + " where drug ='" + sd + "' COLLATE NOCASE").fetchall()
    ...
    if drug_column1 in names:
        ...
        spec_obj_eff1 = conn.execute(
            '''select "%s" from "%s" where country = "%s" COLLATE NOCASE''' % (drug_column1, spec_year_Malaria, selected_country)).fetchall()
        #spec_obj_eff1 = conn.execute(
    ...
    if drug_column2 in names:
        ...
        spec_obj_eff2 = conn.execute(
            '''select "%s" from "%s" where country = "%s" COLLATE NOCASE''' % (drug_column2, spec_year_Malaria, selected_country)).fetchall()
        #spec_obj_eff2 = conn.execute(
...
def getSpecHIV():
    ...
    if selected_year == '2015':
        if "Côte" in selected_country:
            spec_obj = conn.execute(
                "select * from spec_2015_HIV where country like 'Cote%' COLLATE NOCASE").fetchall()
        else:
            spec_obj = conn.execute(
                "select * from spec_2015_HIV where country ='" + selected_country + "' COLLATE NOCASE").fetchall()
        regimen1_obj = conn.execute(
            "select * from spec_2015_HIV_Regimen where Regimens like '%" + sd + "%' and First_line = 1 COLLATE NOCASE").fetchall()
        regimen2_obj = conn.execute(
            "select * from spec_2015_HIV_Regimen where Regimens like '%" + sd + "%' and Second_line = 1 COLLATE NOCASE").fetchall()
        ...
        I_a1 = (adult_daly * adult_coverage * (first_adult_distribution * first_adult_prop * first_adult_eff)) / (
                    1 - adult_coverage * (first_adult_distribution * first_adult_prop * first_adult_eff))
        I_a2 = (adult_daly * adult_coverage * (second_adult_distribution * second_adult_prop * second_adult_eff)) / (
                    1 - adult_coverage * (second_adult_distribution * second_adult_prop * second_adult_eff))
        I_c1 = (child_daly * child_coverage * (first_child_distribution * first_child_prop * first_child_eff)) / (
                    1 - child_coverage * (first_child_distribution * first_child_prop * first_child_eff))
        I_c2 = (child_daly * child_coverage * (second_child_distribution * second_child_prop * second_child_eff)) / (
                    1 - child_coverage * (second_child_distribution * second_child_prop * second_child_eff))
        ...
        if "Côte" in selected_country:
            spec_obj = conn.execute(
                "select * from spec_2013_HIV where country like 'Cote%' COLLATE NOCASE").fetchall()
        else:
            spec_obj = conn.execute(
                "select * from spec_2013_HIV where country ='" + selected_country + "' COLLATE NOCASE").fetchall()
        regimen1_obj_A = conn.execute(
            "select * from spec_2013_HIV_Regimen where Regimens like '%" + sd + "%' and First_line = 1 and grp = 'A' COLLATE NOCASE").fetchall()
        regimen2_obj_A = conn.execute(
            "select * from spec_2013_HIV_Regimen where Regimens like '%" + sd + "%' and Second_line = 1 and grp = 'A' COLLATE NOCASE").fetchall()
        regimen1_obj_B = conn.execute(
            "select * from spec_2013_HIV_Regimen where Regimens like '%" + sd + "%' and First_line = 1 and grp = 'B' COLLATE NOCASE").fetchall()
        regimen2_obj_B = conn.execute(
            "select * from spec_2013_HIV_Regimen where Regimens like '%" + sd + "%' and Second_line = 1 and grp = 'B' COLLATE NOCASE").fetchall()
        ...
        I_a1_A = (adult_daly * adult_coverage * (
                    first_adult_distribution_A * first_adult_prop_A * first_adult_eff_A)) / (
                             1 - adult_coverage * (first_adult_distribution_A * first_adult_prop_A * first_adult_eff_A))
        I_a2_A = (adult_daly * adult_coverage * (
                    second_adult_distribution_A * second_adult_prop_A * second_adult_eff_A)) / (1 - adult_coverage * (
                    second_adult_distribution_A * second_adult_prop_A * second_adult_eff_A))
        I_c1_A = (child_daly * child_coverage * (
                    first_child_distribution_A * first_child_prop_A * first_child_eff_A)) / (
                             1 - child_coverage * (first_child_distribution_A * first_child_prop_A * first_child_eff_A))
        I_c2_A = (child_daly * child_coverage * (
                    second_child_distribution_A * second_child_prop_A * second_child_eff_A)) / (1 - child_coverage * (
                    second_child_distribution_A * second_child_prop_A * second_child_eff_A))
        ...
        I_a1_B = (adult_daly * adult_coverage * (
                    first_adult_distribution_B * first_adult_prop_B * first_adult_eff_B)) / (
                             1 - adult_coverage * (first_adult_distribution_B * first_adult_prop_B * first_adult_eff_B))
        I_a2_B = (adult_daly * adult_coverage * (
                    second_adult_distribution_B * second_adult_prop_B * second_adult_eff_B)) / (1 - adult_coverage * (
                    second_adult_distribution_B * second_adult_prop_B * second_adult_eff_B))
        I_c1_B = (child_daly * child_coverage * (
                    first_child_distribution_B * first_child_prop_B * first_child_eff_B)) / (
                             1 - child_coverage * (first_child_distribution_B * first_child_prop_B * first_child_eff_B))
        I_c2_B = (child_daly * child_coverage * (
                    second_child_distribution_B * second_child_prop_B * second_child_eff_B)) / (1 - child_coverage * (
                    second_child_distribution_B * second_child_prop_B * second_child_eff_B))
        ...
        if "Côte" in selected_country:
            spec_obj = conn.execute(
                "select * from spec_2010_HIV where country like 'Cote%' COLLATE NOCASE").fetchall()
        else:
            spec_obj = conn.execute(
                "select * from spec_2010_HIV where country ='" + selected_country + "' COLLATE NOCASE").fetchall()
        regimen1_obj_A_adult = conn.execute(
            "select * from spec_2010_HIV_Regimen where Regimens like '%" + sd + "%' and First_line = 1 and Adult_eff = 1 and grp = 'A' COLLATE NOCASE").fetchall()
        regimen1_obj_A_child = conn.execute(
            "select * from spec_2010_HIV_Regimen where Regimens like '%" + sd + "%' and First_line = 1 and Children_eff = 1 and grp = 'A' COLLATE NOCASE").fetchall()
        regimen2_obj_A_adult = conn.execute(
            "select * from spec_2010_HIV_Regimen where Regimens like '%" + sd + "%' and Second_line = 1 and Adult_eff = 1 and grp = 'A' COLLATE NOCASE").fetchall()
        regimen2_obj_A_child = conn.execute(
            "select * from spec_2010_HIV_Regimen where Regimens like '%" + sd + "%' and Second_line = 1 and Children_eff = 1 and grp = 'A' COLLATE NOCASE").fetchall()
        regimen1_obj_B_adult = conn.execute(
            "select * from spec_2010_HIV_Regimen where Regimens like '%" + sd + "%' and First_line = 1 and Adult_eff = 1 and grp = 'B' COLLATE NOCASE").fetchall()
        regimen1_obj_B_child = conn.execute(
            "select * from spec_2010_HIV_Regimen where Regimens like '%" + sd + "%' and First_line = 1 and Children_eff = 1 and grp = 'B' COLLATE NOCASE").fetchall()
        regimen2_obj_B_adult = conn.execute(
            "select * from spec_2010_HIV_Regimen where Regimens like '%" + sd + "%' and Second_line = 1 and Adult_eff = 1 and grp = 'B' COLLATE NOCASE").fetchall()
        regimen2_obj_B_child = conn.execute(
            "select * from spec_2010_HIV_Regimen where Regimens like '%" + sd + "%' and Second_line = 1 and Children_eff = 1 and grp = 'B' COLLATE NOCASE").fetchall()
...
def getSpecWorms():
    ...
    print(worm_preSac_coverage)
    print(Efficacy)
    print(prevalence_child)
...
def getSpecOther():
    ...
    # selected_company = request.get_json()["selected_company"]
    selected_specification = request.get_json()["selected_specification"]
    spec_year_NTD = "spec_" + str(selected_year) + "_NTD"

    print(selected_country)
    spec_obj = conn.execute(
        '''select schi_prevalance, schi_coverage, schi_Efficacy, schi_DALY, onch_prevalance, onch_coverage, onch_Efficacy, onch_DALY, LF_prevalance, LF_coverage, LF_Efficacy_DEC, LF_Efficacy_DEC_ALB, LF_Efficacy_IVM_ALB, LF_DALY from "%s" where country = "%s" COLLATE NOCASE''' %(spec_year_NTD, selected_country)).fetchall()
    #if "Côte d'Ivoire" in selected_country:
     #   spec_obj = conn.execute(
      #      f"select schi_prevalance, schi_coverage, schi_Efficacy, schi_DALY, onch_prevalance, onch_coverage, onch_Efficacy, onch_DALY, LF_prevalance, LF_coverage, LF_Efficacy_DEC, LF_Efficacy_DEC_ALB, LF_Efficacy_IVM_ALB, LF_DALY from spec_{selected_year}_NTD where country like 'Cote d%' COLLATE NOCASE").fetchall()
    #else:
     #   spec_obj = conn.execute(
...
def getResults():
    ...
    print(selected_country)
    d["UPDATED SPECIFICATIONS"] = updated_spec["data"]
    d["ORIGINAL SPECIFICATIONS"] = updated_spec["dict"]
    print("**************Orignial/ Updated Specifcations")
    print("Updated", d["UPDATED SPECIFICATIONS"])
    print("\nOriginal", d["ORIGINAL SPECIFICATIONS"])

    total_impact_obj = {}
    original_impacts_obj = {}

    selected_disease_temp = selected_disease
    if selected_disease == 'Onchoceriasis':
        selected_disease_temp = 'Onchocerciasis'
    
    if selected_disease == 'Schistosomiasis':
        selected_disease_temp = 'Schistomasis'
    
    if selected_country == "Côte d'Ivoire":
        selected_country = "Cote d'Ivore"
    
    
    print(selected_country)
    if selected_country:
        impact_obj = conn.execute(
            "select Country, sum(Impact) from results_impact_map" + selected_year + " group by Country").fetchall()
        if selected_country == "Cote d'Ivore":
            code_obj = conn.execute(
                "select ccode from country_codes where country like 'Cote%' COLLATE NOCASE").fetchall()
        else:
            code_obj = conn.execute(
                '''select ccode from country_codes where country = "%s" COLLATE NOCASE''' %(selected_country)).fetchall()
            #code_obj = conn.execute(
            #    "select ccode from country_codes where country ='" + selected_country + "' COLLATE NOCASE").fetchall()
        print(code_obj)
        d["Country Code"] = code_obj[0][0]
        # print(impact_obj)z`
        sum = 0;
        temp = {}
        for item in impact_obj:
            if (item[0] != selected_country):
                sum += item[1]
                temp[item[0]] = item[1]
            else:
                original_impacts_obj["Country"] = item[1]
        total_impact_obj["Country"] = temp
    if selected_drug:
        impact_obj = conn.execute(
            "select Drug,sum(Impact) from results_impact_map" + selected_year + " group by Drug").fetchall()
        drug_disease_obj = conn.execute(
            "select Drug,sum(Impact) from results_impact_map" + selected_year + " where Disease = '" + selected_disease_temp + "'group by Drug").fetchall()
        # print(impact_obj)
        sum = 0
        temp = {}
        temp1 = {}
        for item in impact_obj:
            if (item[0] != selected_drug):
                sum += item[1]
                temp[item[0]] = item[1]
            else:
                original_impacts_obj["Drug"] = item[1]
        for item in drug_disease_obj:
            if (item[0] != selected_drug):
                temp1[item[0]] = item[1]
        total_impact_obj["Drug"] = temp
        d["DRUG/DISEASE"] = temp1
    if selected_disease:

        impact_obj = conn.execute(
            "select Disease,sum(Impact) from results_impact_map" + selected_year + " group by Disease").fetchall()
        # print(impact_obj)
        sum = 0
        temp = {}
        for item in impact_obj:
            if (item[0] != selected_disease_temp):
                sum += item[1]
                temp[item[0]] = item[1]
            else:
                original_impacts_obj["Disease"] = item[1]
        total_impact_obj["Disease"] = temp

    if selected_company:
        impact_obj = conn.execute(
            "select Company,sum(Impact) from results_impact_map" + selected_year + " group by Company").fetchall()
        company_disease_obj = conn.execute(
            "select Company,sum(Impact) from results_impact_map" + selected_year + " where Disease = '" + selected_disease_temp + "' group by Company").fetchall()

        # print("**********************************************")
        # print("Selected Company : " , selected_company)
        # print(impact_obj)
        # print("**********************************************")
        # print(company_disease_obj)
        # print("**********************************************")
        sum = 0
        temp = {}
        temp1 = {}
        ...
        for item in impact_obj:
            # item[0] = item[0].strip();
            if ((item[0].strip()) != (selected_company.strip())):
                sum += item[1]
                temp[item[0]] = item[1]
            else:
                print ("**** COMPANY MATCH ********")
                original_impacts_obj["Company"] = item[1]
        # print(company_disease_obj)
        for item in company_disease_obj:
            if (item[0] != selected_company):
                temp1[item[0]] = item[1]
        # total_impact_obj["Company"] = temp1
        total_impact_obj["Company"] = temp
        d["COMPANY/DISEASE"] = temp1

    # print("      original_impacts_obj          ")
    # print(original_impacts_obj)
    d["TOTAL IMPACTS"] = total_impact_obj
    d["ORIGINAL IMPACTS"] = original_impacts_obj
    ...
    if (selected_disease == "Whipworm"):
        ...
        #     company_obj = conn.execute("select company,whipworm from drugr2015 where whipworm > 0").fetchall()
        ...
        #         "select drug,sum(score)from drug2015 where disease='Whipworm' group by drug").fetchall()
        #     drug_dict = {}
        #     print(selected_drug)
    ...
    if (selected_disease == "Hookworm"):
        ...
        # if (selected_state == 'Company'):
        #     company_obj = conn.execute("select company,hookworm from drugr2015 where hookworm > 0").fetchall()
        #     # print(countries_obj[0])
        #     company_dict = {}
        #     for company in company_obj:
        #         temp = company[0].lstrip();
        ...
        # if (selected_state == 'Drug'):
        #     drug_obj = conn.execute(
        #         "select drug,sum(score)from drug2015 where disease='Hookworm' group by drug").fetchall()
        #     drug_dict = {}
        #     print(selected_drug)
        #     for drgobjs in drug_obj:
        #         # print (drgobjs)
    ...
    if (selected_disease == "Roundworm"):
        ...
        # if (selected_state == 'Company'):
        #     company_obj = conn.execute("select company,roundworm from drugr2015 where roundworm > 0").fetchall()
        #     # print(countries_obj[0])
        #     company_dict = {}
        #     for company in company_obj:
        #         temp = company[0].lstrip();
        ...
        # if (selected_state == 'Drug'):
        #     drug_obj = conn.execute(
        #         "select drug,sum(score)from drug2015 where disease='Roundworm' group by drug").fetchall()
        #     drug_dict = {}
        #     print(selected_drug)
        #     for drgobjs in drug_obj:
        #         # print (drgobjs)
    ...
    if (selected_disease == "Schistosomiasis"):
        # d = {}
        spec_selected_year = "spec_" + str(selected_year) + "_NTD"
        spec_obj = conn.execute('''select schi_prevalance, schi_coverage, schi_Efficacy, schi_DALY from "%s" where country = "%s" COLLATE NOCASE'''%(spec_selected_year, selected_country)).fetchall()

        #if "Cote" in selected_country:
         #   spec_obj = conn.execute(
          #      f"select schi_prevalance, schi_coverage, schi_Efficacy, schi_DALY from spec_{selected_year}_NTD where country like 'Cote' COLLATE NOCASE").fetchall()
        #else:
         #   spec_obj = conn.execute(
        ...
        # if (selected_state == 'Company'):
        #     company_obj = conn.execute(
        #         "select company,schistosomiasis from drugr2015 where schistosomiasis > 0").fetchall()
        #     # print(countries_obj[0])
        #     company_dict = {}
        #     for company in company_obj:
        #         temp = company[0].lstrip();
        ...
        # if (selected_state == 'Disease'):
        #     disease_obj = conn.execute(
        #         "select sum(tb),sum(malaria),sum(hiv),sum(roundworm),sum(hookworm),sum(whipworm), sum(schistosomiasis), sum(onchoceriasis), sum(lf) from country2015").fetchall()
        #     disease_dict = {"TB": 0, "Malaria": 0, "HIV": 0, "Roundworm": 0, "Hookworm": 0, "Whipworm": 0,
        #                     "Schistosomiasis": 0, "Onchoceriasis": 0, "LF": 0}
        #     disease_dict["TB"] = round(disease_obj[0][0], 2)
        #     disease_dict["Malaria"] = round(disease_obj[0][1], 2)
        ...
        # if (selected_state == 'Drug'):
        #     drug_obj = conn.execute(
        #         "select drug,sum(score)from drug2015 where disease='Schistosomiasis' group by drug").fetchall()
        #     drug_dict = {}
        #     print(selected_drug)
        #     for drgobjs in drug_obj:
        #         # print (drgobjs)
    ...
    if (selected_disease == "Onchoceriasis"):
        ...
        # if (selected_state == 'Company'):
        #     company_obj = conn.execute("select company,onchoceriasis from drugr2015 where onchoceriasis > 0").fetchall()
        #     # print(countries_obj[0])
        #     company_dict = {}
        #     for company in company_obj:
        #         temp = company[0].lstrip();
        ...
        # if (selected_state == 'Drug'):
        #     drug_obj = conn.execute(
        #         "select drug,sum(score)from drug2015 where disease='onchoceriasis' group by drug").fetchall()
        #     drug_dict = {}
        #     print(selected_drug)
        #     for drgobjs in drug_obj:
        #         # print (drgobjs)
    ...
    if (selected_disease == "LF"):
        ...
        spec_year_NTD = "spec_" + str(selected_year) + "_NTD"
        spec_obj = conn.execute(
            '''select LF_prevalance, LF_coverage, LF_Efficacy_DEC, LF_Efficacy_DEC_ALB, LF_Efficacy_IVM_ALB, LF_DALY from "%s" where country = "%s" COLLATE NOCASE''' %(spec_year_NTD, selected_country)).fetchall()
        #spec_obj = conn.execute(
        ...
        # if (selected_state == 'Company'):
        #     company_obj = conn.execute("select company,lf from drugr2015 where lf > 0").fetchall()
        #     # print(countries_obj[0])
        #     company_dict = {}
        #     for company in company_obj:
        #         temp = company[0].lstrip();
        ...
        #
        # if (selected_state == 'Drug'):
        #     drug_obj = conn.execute("select drug,sum(score)from drug2015 where disease='if' group by drug").fetchall()
        #     drug_dict = {}
        #     print(selected_drug)
        #     for drgobjs in drug_obj:
        #         # print (drgobjs)
    ...
    if (selected_disease == "HIV"):
        ...
        if (selected_year == '2015'):
            spec_obj = conn.execute('''
                select * from spec_2015_HIV where country = "%s" COLLATE NOCASE'''%(selected_country)
                ).fetchall()
            #spec_obj = conn.execute(
              #  "select * from spec_2015_HIV where country ='" + selected_country + "' COLLATE NOCASE").fetchall()
            regimen1_obj = conn.execute(
            ...
            newI_a1 = (adult_daly * adult_coverage * (
                        first_adult_distribution * first_adult_prop * first_adult_eff)) / (
                                  1 - adult_coverage * (first_adult_distribution * first_adult_prop * first_adult_eff))
            newI_a2 = (adult_daly * adult_coverage * (
                        second_adult_distribution * second_adult_prop * second_adult_eff)) / (1 - adult_coverage * (
                        second_adult_distribution * second_adult_prop * second_adult_eff))
            newI_c1 = (child_daly * child_coverage * (
                        first_child_distribution * first_child_prop * first_child_eff)) / (
                                  1 - child_coverage * (first_child_distribution * first_child_prop * first_child_eff))
            newI_c2 = (child_daly * child_coverage * (
                        second_child_distribution * second_child_prop * second_child_eff)) / (1 - child_coverage * (
                        second_child_distribution * second_child_prop * second_child_eff))
        ...
        if (selected_year == '2013'):
            spec_obj = conn.execute('''
                select * from spec_2013_HIV where country = "%s" COLLATE NOCASE'''%(selected_country)
                ).fetchall()
            #spec_obj = conn.execute(
             #   "select * from spec_2013_HIV where country ='" + selected_country + "' COLLATE NOCASE").fetchall()
            regimen1_obj_A = conn.execute(
            ...
            I_a1_A = (adult_daly * adult_coverage * (
                        first_adult_distribution_A * first_adult_prop_A * first_adult_eff_A)) / (1 - adult_coverage * (
                        first_adult_distribution_A * first_adult_prop_A * first_adult_eff_A))
            I_a2_A = (adult_daly * adult_coverage * (
                        second_adult_distribution_A * second_adult_prop_A * second_adult_eff_A)) / (
                                 1 - adult_coverage * (
                                     second_adult_distribution_A * second_adult_prop_A * second_adult_eff_A))
            I_c1_A = (child_daly * child_coverage * (
                        first_child_distribution_A * first_child_prop_A * first_child_eff_A)) / (1 - child_coverage * (
                        first_child_distribution_A * first_child_prop_A * first_child_eff_A))
            I_c2_A = (child_daly * child_coverage * (
                        second_child_distribution_A * second_child_prop_A * second_child_eff_A)) / (
                                 1 - child_coverage * (
                                     second_child_distribution_A * second_child_prop_A * second_child_eff_A))
            I_a1_B = (adult_daly * adult_coverage * (
                        first_adult_distribution_B * first_adult_prop_B * first_adult_eff_B)) / (1 - adult_coverage * (
                        first_adult_distribution_B * first_adult_prop_B * first_adult_eff_B))
            I_a2_B = (adult_daly * adult_coverage * (
                        second_adult_distribution_B * second_adult_prop_B * second_adult_eff_B)) / (
                                 1 - adult_coverage * (
                                     second_adult_distribution_B * second_adult_prop_B * second_adult_eff_B))
            I_c1_B = (child_daly * child_coverage * (
                        first_child_distribution_B * first_child_prop_B * first_child_eff_B)) / (1 - child_coverage * (
                        first_child_distribution_B * first_child_prop_B * first_child_eff_B))
            I_c2_B = (child_daly * child_coverage * (
                        second_child_distribution_B * second_child_prop_B * second_child_eff_B)) / (
                                 1 - child_coverage * (
        ...
        if (selected_year == '2010'):
            spec_obj = conn.execute('''
                select * from spec_2010_HIV where country = "%s" COLLATE NOCASE'''%(selected_country)
                ).fetchall()
            #spec_obj = conn.execute(
             #   "select * from spec_2010_HIV where country ='" + selected_country + "' COLLATE NOCASE").fetchall()
            regimen1_obj_A_adult = conn.execute(
        ...
        #     # countries_obj = conn.execute("select country,whipworm from country2015").fetchall()
        #     countries_obj = conn.execute(
        #         "select country,score from countryDrug{selected_year} where drug='" + selected_drug + "' and score > 0.0").fetchall()
        #     countries_dict = {}
        #     for countries in countries_obj:
        #         temp = countries[0].lstrip();
        #         if (temp != selected_country):
    ...
    # Daniel - Calculations for impact score as by 3.2
    calc = {}
    selected_disease_temp = selected_disease
    if selected_disease == 'Onchoceriasis':
        selected_disease_temp = 'Onchocerciasis'
    ...
    orig_score = d["Original Impact Score"]
    new_score = d["New Impact Score"]
    if (orig_score > 0):
        percChange = new_score / orig_score
    else:
        percChange = new_score

    if selected_country == "Cote d'Ivore":
        selected_country = "Cote d'Ivoire"

    if selected_company and selected_country and selected_disease and selected_drug:

        
        sql_instruction = "select Impact from results_impact_map" + selected_year + " where trim(Company)=trim('" + selected_company + "')"
        sql_instruction = sql_instruction + ''' AND trim(Country) = trim("%s")''' %(selected_country)
        #if 'Cote' in selected_country:
         #   sql_instruction = sql_instruction + " AND Country like 'Cote%'"
        #else:
         #   sql_instruction = sql_instruction + " AND Country=trim('" + selected_country + "')"
        sql_instruction = sql_instruction + " AND Disease=trim('" + selected_disease_temp + "')"
        sql_instruction = sql_instruction + " AND Drug=trim('" + selected_drug + "') "
        print("TEST")
        print(sql_instruction)
        print(conn.execute(sql_instruction).fetchall())
        orig_score = conn.execute(sql_instruction).fetchall()[0][0]

    new_score = orig_score * percChange
    d["Original Impact Score"] = orig_score
    d["New Impact Score"] = new_score
    results_impact_map_year = "results_impact_map" + str(selected_year)
    if selected_country:
        if 'Cote' in selected_country:
            impact_obj = conn.execute(
                "select sum(Impact) from results_impact_map" + selected_year + " where Country like 'Cote%'").fetchall()
        else:
            impact_obj = conn.execute(
                '''select sum(Impact) from "%s" where Country = "%s" ''' %(results_impact_map_year, selected_country)).fetchall()
            #impact_obj = conn.execute(
            #    "select sum(Impact) from results_impact_map" + selected_year + " where Country='" + selected_country + "'").fetchall()

        sum = 0;

        total_score = impact_obj[0][0]
        print("orig_score", orig_score)
        print("new_score", new_score)
        print("total_score", total_score)
        res = (total_score - orig_score) + new_score
        calc["Country"] = res

    if selected_drug:
        impact_obj = conn.execute(
            "select sum(Impact) from results_impact_map" + selected_year + " where Drug='" + selected_drug + "'").fetchall()
        sum = 0;
    ...
    if selected_disease:
        impact_obj = conn.execute(
            "select sum(Impact) from results_impact_map" + selected_year + " where Disease='" + selected_disease_temp + "'").fetchall()
        sum = 0;
        total_score = impact_obj[0][0]
        # print("orig_score",orig_score)
        # print("new_score",new_score)
    ...
    if selected_company:
        ...
        impact_obj = conn.execute(
            "select sum(Impact) from results_impact_map" + selected_year + " where Company=trim('" + selected_company + "')").fetchall()
        sum = 0;
        total_score = impact_obj[0][0]
        # print("orig_score",orig_score)
        # print("new_score",new_score)
