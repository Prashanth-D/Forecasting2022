...
def getSpecHIV():
    ...
    if selected_year == '2015':
        ...
        regimen1_obj_A_adult = conn.execute(
            "select * from spec_2010_HIV_Regimen where Regimens like '%" + sd + "%' and First_line = 1 and adult = 1 and grp = 'A' COLLATE NOCASE").fetchall()
        regimen1_obj_A_child = conn.execute(
            "select * from spec_2010_HIV_Regimen where Regimens like '%" + sd + "%' and First_line = 1 and child = 1 and grp = 'A' COLLATE NOCASE").fetchall()
        regimen2_obj_A_adult = conn.execute(
            "select * from spec_2010_HIV_Regimen where Regimens like '%" + sd + "%' and Second_line = 1 and adult = 1 and grp = 'A' COLLATE NOCASE").fetchall()
        regimen2_obj_A_child = conn.execute(
            "select * from spec_2010_HIV_Regimen where Regimens like '%" + sd + "%' and Second_line = 1 and child = 1 and grp = 'A' COLLATE NOCASE").fetchall()
        regimen1_obj_B_adult = conn.execute(
            "select * from spec_2010_HIV_Regimen where Regimens like '%" + sd + "%' and First_line = 1 and adult = 1 and grp = 'B' COLLATE NOCASE").fetchall()
        regimen1_obj_B_child = conn.execute(
            "select * from spec_2010_HIV_Regimen where Regimens like '%" + sd + "%' and First_line = 1 and child = 1 and grp = 'B' COLLATE NOCASE").fetchall()
        regimen2_obj_B_adult = conn.execute(
            "select * from spec_2010_HIV_Regimen where Regimens like '%" + sd + "%' and Second_line = 1 and adult = 1 and grp = 'B' COLLATE NOCASE").fetchall()
        regimen2_obj_B_child = conn.execute(
            "select * from spec_2010_HIV_Regimen where Regimens like '%" + sd + "%' and Second_line = 1 and child = 1 and grp = 'B' COLLATE NOCASE").fetchall()
...
def getSpecWorms():
    ...
    for each_spec in selected_specification:
        ...
        if each_spec == "Prevalence for 1-4 year olds":
            d["Prevalence for 1-4 year olds"] = prevalence_child * 100
        if each_spec == "Prevalence for 5-14 year olds":
            d["Prevalence for 5-14 year olds"] = prevalence_adult * 100

        # Set treatment coverage values
        if each_spec == "Pre-Sac Treatment Coverage":
            d["Pre-Sac Treatment Coverage"] = worm_preSac_coverage * 100
        if each_spec == "SAC Treatment Coverage":
            d["SAC Treatment Coverage"] = worm_Sac_coverage * 100

        # Set efficacy values
        if each_spec == "Efficacy":
            d["Efficacy"] = Efficacy * 100
...
def getSpecOther():
    ...
    spec_obj = conn.execute(
        "select schi_prevalance, schi_coverage, schi_Efficacy, schi_DALY, onch_prevalance, onch_coverage, onch_Efficacy, onch_DALY, LF_prevalance, LF_coverage, LF_Efficacy_DEC, LF_Efficacy_DEC_ALB, LF_Efficacy_IVM_ALB, LF_DALY from spec_2015_NTD where country ='" + selected_country + "' COLLATE NOCASE").fetchall()
    ...
    for each_spec in selected_specification:
        ...
        if each_spec == "Prevalence":
            d["Prevalence"] = prevalence * 100

        # Set treatment coverage value
        if each_spec == "Treatment Coverage":
            d["Treatment Coverage"] = treatment_coverage * 100
...
def getResults():
    ...
    if selected_drug:
        ...
        drug_disease_obj = conn.execute(
            "select Drug,sum(Impact)from results_impact_map" + selected_year + " where Disease = '" + selected_disease + "'group by Drug").fetchall()
        # print(impact_obj)
        sum = 0
        temp = {}
        temp1 = {}
    ...
    if selected_company:
        ...
        company_disease_obj = conn.execute(
            "select Company,sum(Impact)from results_impact_map" + selected_year + " where Disease = '" + selected_disease + "' group by Company").fetchall()
        # print(impact_obj)
        sum = 0
        temp = {}
        temp1 = {}
    ...
    if selected_company and selected_country and selected_disease and selected_drug:
        sql_instruction = "select Impact from results_impact_map" + selected_year + " where Company=trim('" + selected_company + "')"
        sql_instruction = sql_instruction + " AND Country=trim('" + selected_country + "')"
        sql_instruction = sql_instruction + " AND Disease=trim('" + selected_disease + "')"
    ...
    if selected_disease:
        impact_obj = conn.execute(
            "select sum(Impact)from results_impact_map" + selected_year + " where Disease='" + selected_disease + "'").fetchall()
        sum = 0;
        total_score = impact_obj[0][0]
        # print("orig_score",orig_score)
        # print("new_score",new_score)
