...
def getCountries():
    ...
    if (requestJSON is not None):
        if "selected_disease" in requestJSON:
            ...
            conn = connect_db()
            obj = conn.execute("select country from country2015 where " + string_disease_select + "> 0").fetchall()
...
def getDrugByDisease():
    ...
    if ((requestJSON is not None) and ("selected_disease" in requestJSON) and ("selected_country" in requestJSON)):
        ...
        if selected_country == "Lao People's Democratic Republic":
            drug_obj_by_cntry  = conn.execute("select distinct drug from countryDrug2015 where country like 'Lao People%' and score>0").fetchall()
        else:
...
def getDrugByDiseaseCmpny():
    ...
    drugsToSelect = []
    conn = connect_db()
    drug_obj_by_dis = conn.execute("select distinct drug from drug2015 where disease ='"+selected_disease+"' COLLATE NOCASE and company ='"+selected_company+"' COLLATE NOCASE").fetchall()
    #drug_obj = conn.execute("select drug from drug2015 where disease ='"+selected_disease+"' COLLATE NOCASE").fetchall()

    if selected_country == "Lao People's Democratic Republic":
        drug_obj_by_cntry = conn.execute(
            "select distinct drug from countryDrug2015 where country like 'Lao People%' and score>0").fetchall()
    else:
        drug_obj_by_cntry = conn.execute(
            "select distinct drug from countryDrug2015 where country='" + selected_country + "' and score>0").fetchall()
...
def getSpecification():
    ...
    spec_obj_NTDs = conn.execute("select disease from drug2015 where drug ='" + selected_drug + "' COLLATE NOCASE").fetchall()
    print(spec_obj_NTDs)
...
def getResults():
    ...
    if(selected_disease == "Whipworm"):       
        ...
        if (selected_state == 'Disease'):
            disease_obj = conn.execute("select tb , malaria, hiv, roundworm , hookworm , whipworm ,  schistosomiasis ,  onchoceriasis , lf from country2015 where country = ' " + selected_country + "' ").fetchall()
        ...
        if (selected_state == 'Drug'):
            drug_obj = conn.execute("select drug,sum(score)from drug2015 where disease='Whipworm' group by drug").fetchall()
            drug_dict= {}
            print(selected_drug)
    ...
    if(selected_disease == "Hookworm"):
        ...
        if (selected_state == 'Drug'):
            drug_obj = conn.execute("select drug,sum(score)from drug2015 where disease='Hookworm' group by drug").fetchall()
            drug_dict= {}
            print(selected_drug)
            for drgobjs in drug_obj:
                # print (drgobjs)
    ...
    if(selected_disease == "Roundworm"):
        ...
        if (selected_state == 'Drug'):
            drug_obj = conn.execute("select drug,sum(score)from drug2015 where disease='Roundworm' group by drug").fetchall()
            drug_dict= {}
            print(selected_drug)
            for drgobjs in drug_obj:
                # print (drgobjs)
    ...
    if(selected_disease == "Schistosomiasis"):
        ...
        if (selected_state == 'Company'):
            company_obj = conn.execute("select company,schistosomiasis from drugr2015 where schistosomiasis > 0").fetchall()
            # print(countries_obj[0])
            company_dict= {}
            for company in company_obj:
                temp = company[0].lstrip();
        ...
        if (selected_state == 'Drug'):
            drug_obj = conn.execute("select drug,sum(score)from drug2015 where disease='Schistosomiasis' group by drug").fetchall()
            drug_dict= {}
            print(selected_drug)
            for drgobjs in drug_obj:
                # print (drgobjs)
    ...
    if(selected_disease == "Onchoceriasis"):
        ...
        if (selected_state == 'Drug'):
            drug_obj = conn.execute("select drug,sum(score)from drug2015 where disease='onchoceriasis' group by drug").fetchall()
            drug_dict= {}
            print(selected_drug)
            for drgobjs in drug_obj:
                # print (drgobjs)
    ...
    if (selected_disease == "Malaria"):
        ...
        if (selected_state == 'Disease'):
            disease_obj = conn.execute("select tb , malaria, hiv, roundworm , hookworm , whipworm , schistosomiasis , onchoceriasis , lf from country2015 WHERE country =  ' " +  selected_country + "' ").fetchall() #Fetch data only for specific country
            disease_dict= {"TB":0, "Malaria":0, "HIV":0, "Roundworm":0, "Hookworm":0, "Whipworm":0,
                           "Schistosomiasis":0, "Onchoceriasis":0, "LF":0}
            print(disease_obj)
            disease_dict["TB"] = round(disease_obj[0][0],2)
        ...
        if (selected_state == 'Drug'):
            drug_obj = conn.execute("select drug,sum(score)from drug2015 where disease='Malaria' group by drug").fetchall()
            drug_dict= {}
            print(selected_drug)
            for drgobjs in drug_obj:
                # print (drgobjs)
